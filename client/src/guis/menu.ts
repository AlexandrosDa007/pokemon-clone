...
export class Menu extends UIControl {
  ...
  constructor() {
    super(
      ...
      0.28,
      0.8,
      Settings.CANVAS_WIDTH - 0.3 * Settings.CANVAS_WIDTH,
      0,
      () => {},
    );
    this.uiControls = MENU_ITEMS.map(
      (item, index) =>
        new MenuItem(item.id, item.name, this, item.action, index),
    );
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    ctx.lineWidth = 2;
    ctx.drawImage(
      SpriteLoader.SPRITES.ARROW_RIGHT.image,
      0,
      0,
      SpriteLoader.SPRITES.ARROW_RIGHT.width,
      SpriteLoader.SPRITES.ARROW_RIGHT.height,
      this.x,
      this.y + 16 + this.cursor * 0.1 * Settings.CANVAS_HEIGHT,
      32,
      32,
    );
...
class MenuItem extends UIControl {
  ...
  constructor(
    ...
    super(
      ...
      0.1,
      parent.x,
      index * (Settings.CANVAS_HEIGHT * 0.1),
      action,
    );
    this.name = name;
    this.parent = parent;
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.fillText(
      this.name,
      this.parent.x + MENU_ITEM_OFFSET_X,
      this.y + (this.height / 2) * Settings.CANVAS_HEIGHT,
    );
  }
}
