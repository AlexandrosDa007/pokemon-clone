...
import {
  ...
  EncounterAction,
  EncounterTurn,
} from '@shared/models/db-encounter';
...
export class Encounter {
  ...
  constructor(uid: string, socket: Socket) {
    ...
    this.socket = socket;
    this.socket.on(
      SOCKET_EVENTS.ENCOUNTER_ACKNOWLEDGED,
      (turnAcknowledged: number) => {
  ...
  update(lastKey: GameKeyCode | null) {
    ...
    // map key to action
    const action =
      lastKey === 'FIRST_ATTACK' ? EncounterAction.ATTACK : EncounterAction.RUN;
    ...
    if (this.encounterState.turn === EncounterTurn.PLAYER) {
      ...
      // wild pokemon turn
      const newState = HANDLE_ATTACK(
        this.encounterState,
        this.encounterState.pokemon.moves[0],
      );
      this.encounterState = newState;
      return newState;
    } else {
...
function HANDLE_ATTACK(encounterState: DbEncounter, attackId: Attack) {
  ...
  if (turn === EncounterTurn.PLAYER) {
    // TODO: make deep copy
    const moveIndex = encounterState.pokemon.moves.findIndex(
      (item) => item === attackId,
    );
    if (moveIndex === -1) {
      throw new Error(`No such move`);
    }
    const newPokemonPP = encounterState.pokemon.pp.map((p, index) =>
      index === moveIndex && !!p ? p - 1 : p,
    );
    return {
      ...encounterState,
      turnIndex: encounterState.turnIndex + 1,
      turn:
        encounterState.turn === EncounterTurn.PLAYER
          ? EncounterTurn.PLAYER
          : EncounterTurn.PLAYER,
      pokemon: {
        ...encounterState.pokemon,
        moves: [...encounterState.pokemon.moves],
        pp: newPokemonPP,
      },
      pokemonOut: {
