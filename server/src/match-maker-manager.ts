...
export class MatchMakerManager {
    ...
    static invitations: Record<string, {
        uid: string;
        playerName: string;
        timestamp: number;
        expiresAt: number;
    ...
    static sendInvitation(player: Player, playerUid: string) {
        ...
        const sendNotification = () => {
            ...
            IO.to(receiver.socket.id).emit(SOCKET_EVENTS.BATTLE_INVITES, receiverInvitations);
        ...
        if (receiverInvitations.find(inv => inv.uid === player.uid)) {
            receiverInvitations.find(inv => inv.uid === player.uid)!.timestamp = Date.now();
            sendNotification();
            return;
        }
